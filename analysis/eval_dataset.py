...
def extracted_test(file_dir, data):
    ...
    for metric in metrics:
        ...
        if "mm" in data:
            ...
            df = df.loc[df["Metric"].isin(["{}[optic]".format(metric)])]
        elif "prostate" in data:
            df = df.loc[df["Metric"].isin(["{}[{}]".format(metric, data)])]
        elif "hippocampus" in data:
            df = df.loc[df["Metric"].isin(["{}[{}]".format(metric, data)])]
...
if __name__ == "__main__":
    ...
    dataset = "mm-r"  # mm-i or mm-o or mm-r or prostate or hippocampus or optic-i or optic-o
    approach_path_list = []
    approach_list = []
