...
if __name__ == "__main__":
    file_dirs = "/Share8/zhuzhanshi/CauAug/storage/exp"
    out_dir = "/Share8/zhuzhanshi/CauAug/storage/figure"
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    dataset = "optic-o"  # mm-i or mm-o or mm-r or hippocampus or prostate or optic-i or optic-o
    metric = "Mean_ScoreDice"  # Mean_ScoreDice, Mean_ScoreIoU, Mean_ScoreHausdorff, Std_ScoreDice, Std_ScoreIoU, Std_ScoreHausdorff
    # metric = "Mean_ScoreHausdorff"
    approach_path_list = []
    approach_list = []
    table_dict = {}
    ...
    for name in os.listdir(file_dirs):
        if dataset in name:
            ...
            df.to_csv(
                os.path.join(out_dir, dataset + "_tem.csv"),
                index=False,
            )
            print("avg:", avg)
