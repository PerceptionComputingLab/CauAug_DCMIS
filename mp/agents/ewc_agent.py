        ...
        for data in tqdm(train_dataloader, disable=True):
            ...
            if self.model.importance_weights != None:
                ...
                for param_old, param_new, weights in zip(
                    self.model.unet_old.parameters(), self.model.unet_new.parameters(), self.model.importance_weights
                ):
        ...
        if print_run_loss:
            print(
                "\nrunning loss: {} - ewc: {} - time/epoch {}".format(
                    acc.mean("loss"), acc.mean("loss_ewc"), round(time.time() - start_time, 4)
                )
            )
