...
class RefreshAgent(SegmentationAgent):
    ...
    def train(
        ...
        for epoch in range(init_epoch, nr_epochs):
            ...
            acc = self.perform_training_epoch(loss_f, train_dataloader, config, print_run_loss=print_run_loss)
            if val_best:
    ...
    def perform_training_epoch(self, loss_f, train_dataloader, config, print_run_loss=False):
        ...
        # temp model
        temp_model = copy.deepcopy(self.model.unet_new).to(self.device)
        temp_model.train()
        temp_optim = torch.optim.Adam(temp_model.parameters(), lr=0.01)
        ...
        for data in tqdm(train_dataloader, disable=True):
            ...
            # unlearn stage
            if self.model.unet_old is not None:
                temp_model.load_state_dict(self.model.unet_new.state_dict())
                outputs_temp = temp_model(inputs)
                outputs_temp = softmax(outputs_temp).clamp(min=1e-08, max=1.0 - 1e-08)
                loss_unlearn = loss_f(outputs_temp, targets)
                ...
                with torch.no_grad():
                    for i, (name, param) in enumerate(self.model.unet_new.named_parameters()):
                        temp_param = dict(temp_model.named_parameters())[name]
                        weight_update = temp_param - param
                        param_norm = param.norm()
                        update_norm = weight_update.norm() + 1e-20
                        norm_update = param_norm / update_norm * weight_update
                        identity = torch.ones_like(param)
                        # fisher information matrix
                        if self.model.importance_weights is not None:
                            fish_val = self.model.importance_weights[i]
                        else:
                            fish_val = identity
                        noise = 0.001 * torch.randn_like(param)
                        adjustment = 1.0 / (identity + 0.0001 * fish_val)
                        param.add_(0.00001 * (adjustment * (norm_update + noise)))
        ...
        if print_run_loss:
            print(
                "\nrunning loss: {} - seg: {} - time/epoch {}".format(
                    acc.mean("loss"), acc.mean("loss_seg"), round(time.time() - start_time, 4)
                )
            )
