...
from mp.utils.dataaug import drfr_mutual_augment, combined_blur_batch, batch_augment_vectorized


def augment_data(inputs, aug_type):
    if aug_type == "none":
        pass
    elif aug_type == "standard":
        inputs = batch_augment_vectorized(inputs)
    elif aug_type == "drfr":
        inputs = drfr_mutual_augment(inputs)
    elif aug_type == "blur":
        inputs = combined_blur_batch(inputs)
    else:
...
class KDAgent(SegmentationAgent):
    ...
    def perform_training_epoch(self, loss_f, train_dataloader, config, epoch, print_run_loss=False):
        ...
        for data in tqdm(train_dataloader, disable=True):
            ...
            inputs = augment_data(inputs, config["aug_type"])
